...
namespace SampleCsCommands
{
  public class SampleCsCreateNestedBlock : Command
  {
    ...
    protected override Result RunCommand(RhinoDoc doc, RunMode mode)
    {
      ...
      ArcCurve curve0 = new ArcCurve(circle);

      // Add an instance defintion that uses the circle curve
      Rhino.DocObjects.ObjectAttributes attrib = doc.CreateDefaultAttributes();
      int idef0_index = doc.InstanceDefinitions.Add("Circle", "Circle", Point3d.Origin, curve0, attrib);

      // Create a reference to the instance definition
      System.Guid idef0_id = doc.InstanceDefinitions[idef0_index].Id;
      InstanceReferenceGeometry iref0 = new InstanceReferenceGeometry(idef0_id, Transform.Identity);
      ...
      rect[4] = rect[0];
      PolylineCurve curve1 = new PolylineCurve(rect);

      // Add another instance definition that uses the polyline curve
      // and the instance reference
      GeometryBase[] geometry = new GeometryBase[] { curve1, iref0 };
      Rhino.DocObjects.ObjectAttributes[] attributes = new[] { attrib, attrib };
      int idef1_index = doc.InstanceDefinitions.Add("Rectangle and Circle", "Rectangle and Circle", Point3d.Origin, geometry, attributes);
