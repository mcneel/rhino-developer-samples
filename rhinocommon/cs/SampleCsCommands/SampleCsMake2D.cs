...
namespace SampleCsCommands
{
  public class SampleCsMake2D : Command
  {
    ...
    protected override Result RunCommand(RhinoDoc doc, RunMode mode)
    {
      ...
      HiddenLineDrawing hld = HiddenLineDrawing.Compute(hld_params, true);

      if (hld != null)
      {
        Transform flatten = Transform.PlanarProjection(Plane.WorldXY);
        BoundingBox page_box = hld.BoundingBox(true);
        Vector2d delta_2d = new Vector2d(0, 0);
        delta_2d = delta_2d - new Vector2d(page_box.Min.X, page_box.Min.Y);
        Transform delta_3d = Transform.Translation(new Vector3d(delta_2d.X, delta_2d.Y, 0.0));
        flatten = delta_3d * flatten;

        ObjectAttributes h_attribs = new ObjectAttributes { Name = "H" };
        ObjectAttributes v_attribs = new ObjectAttributes { Name = "V" };

        foreach (HiddenLineDrawingSegment hld_curve in hld.Segments)
        {
          if (hld_curve?.ParentCurve == null || hld_curve.ParentCurve.SilhouetteType == SilhouetteType.None)
            continue;

          Curve crv = hld_curve.CurveGeometry.DuplicateCurve();
          if (crv != null)
          {
        ...
        foreach (HiddenLineDrawingPoint hld_pt in hld.Points)
        {
          if (hld_pt == null)
            continue;

          Point3d pt = hld_pt.Location;
          if (pt.IsValid)
          {
