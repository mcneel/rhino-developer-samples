...
IMPLEMENT_DYNAMIC( CBlockWalkDialog, CRhinoDialog )

CBlockWalkDialog::CBlockWalkDialog( CWnd* pParent, CRhinoDoc& doc )
: CRhinoDialog(CBlockWalkDialog::IDD, pParent)
, m_doc(doc)
, m_idef(0)
{
}

CBlockWalkDialog::~CBlockWalkDialog()
{
}

void CBlockWalkDialog::DoDataExchange( CDataExchange* pDX )
{
...
BEGIN_MESSAGE_MAP( CBlockWalkDialog, CRhinoDialog )
  ON_NOTIFY(TVN_SELCHANGED, IDC_BLOCK_TREE, &CBlockWalkDialog::OnSelChangeBlockTree)
  ON_CBN_SELCHANGE(IDC_COMBO_VIEW, &CBlockWalkDialog::OnSelChangeComboView)
  ON_CBN_SELCHANGE(IDC_COMBO_DISPLAY, &CBlockWalkDialog::OnSelChangeComboDisplay)
END_MESSAGE_MAP()

BOOL CBlockWalkDialog::OnInitDialog()
{
...
void CBlockWalkDialog::AddInstanceDefinitionToTreeCtrl( const CRhinoInstanceDefinition* idef, HTREEITEM hParent, HTREEITEM hInsertAfter, bool bAddDef  )
{
    ...
    for( i = 0; i < objects.Count(); i++ )
    {
      ...
      if( p )
      {
        ...
        if( q )
        {
          ...
          m_tree.SetItemData( hItem, (DWORD_PTR)-((int)obj->m_runtime_object_serial_number) );
...
void CBlockWalkDialog::OnSelChangeBlockTree( NMHDR* pNMHDR, LRESULT* pResult )
{
  LPNMTREEVIEW tv = reinterpret_cast<LPNMTREEVIEW>( pNMHDR );
...
void CBlockWalkDialog::OnSelChangeComboView()
{
...
void CBlockWalkDialog::OnSelChangeComboDisplay()
{
  int n = m_display_combo.GetCurSel();
  if( n != CB_ERR )
  {
    ON::display_mode dm = ON::DisplayMode( ++n );
    m_preview.SetDisplayMode( dm, true );
  }
}
