...
bool CCommandSampleSyncRenderColors::SychronizeDiffuseColorWithDisplayColor( CRhinoDoc& doc, const CRhinoObject* obj )
{
  ...
  if( color_source == ON::color_from_layer )
  {
    ...
    if( material_index < 0 )
    {
      ...
      material_index = doc.m_material_table.AddMaterial( material );
      if( material_index >= 0 )
      {
        ON_Layer new_layer( layer );
        new_layer.SetRenderMaterialIndex( material_index );
        if( !doc.m_layer_table.ModifyLayer(new_layer, layer.LayerIndex()) )
          return false;
      }
    }
  }
  ...
  if( !doc.m_material_table.ModifyMaterial(new_material, material.MaterialIndex(), FALSE) )
    return false;
