...
CRhinoCommand::result CCommandSampleDumpMaterialUserData::RunCommand( const CRhinoCommandContext& context )
{
  ...
  for (int i = 0; i < material_count; i++)
  {
    ...
    ON_wString str;
    ON_UuidToString(material.m_material_id, str);
    theApp.Print(L"  Material ID = %s\n", (const wchar_t*)str);

    str = L"<none>";
    if (!material.m_material_name.IsEmpty())
      str = material.m_material_name;
    theApp.Print(L"  Material Name = %s\n", (const wchar_t*)str);

    str = L"<none>";
    if (ON_UuidIsNotNil(material.m_plugin_id))
      ON_UuidToString(material.m_plugin_id, str);
