...
)
{
  ...
  if (
    nullptr == pick_point ||
    m_initial_radius_dc.m_pick_result.m_gumball_mode != gb_mode_nothing ||
    m_initial_height_dc.m_pick_result.m_gumball_mode != gb_mode_nothing
    )
  {
  ...
  vp.GetPickXform(pick_point->x, pick_point->y, pick_context.m_pick_region.m_xform);
...
)
{
  ...
  ON_Line world_line(ON_3dPoint::UnsetPoint,ON_3dPoint::UnsetPoint);
  if (vp.VP().GetFrustumLine(pick_point->x, pick_point->y, world_line))
  {
    ...
    if (fabs(len2) >= ON_ZERO_TOLERANCE && fabs(len1) >= ON_ZERO_TOLERANCE)
    {
      ...
      if (m_initial_radius_dc.m_pick_result.m_gumball_mode != gb_mode_nothing)
      {
        m_initial_radius_dc.UpdateGumball(point, world_line);
        m_cylinder.SetRadius(m_cylinder.InitialRadius() * scale);
      }
      else if (m_initial_height_dc.m_pick_result.m_gumball_mode != gb_mode_nothing)
      {
...
{
  m_cylinder.Draw(*vp.DisplayPipeline());
...
{
  SetBasePoint(m_cylinder.Center());
...
public:
  ...
  const wchar_t* EnglishCommandName() { return L"SampleGumballCylinder"; }
