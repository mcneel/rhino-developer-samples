...
void CRhGetPolylineSegment::DynamicDraw( HDC hdc, CRhinoViewport& vp, const ON_3dPoint& pt )
{
  if( m_draw && m_curve )
  {
    ON_Color saved_color = vp.SetDrawColor( RhinoApp().AppSettings().SelectedObjectColor() );
    vp.DrawLine( m_curve->m_pline[m_index0], m_curve->m_pline[m_index1] );
...
bool CRhGetPolylineObject::CustomGeometryFilter(
    const CRhinoObject* object,
    const ON_Geometry* geometry,
    ON_COMPONENT_INDEX component_index
    ) const
