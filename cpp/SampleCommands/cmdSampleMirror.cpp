...
void CMirror2Xform::DynamicDraw( HDC hdc, CRhinoViewport& vp, const ON_3dPoint& point )
{
  vp.DrawLine( m_corners[0], m_corners[1] );
  vp.DrawLine( m_corners[1], m_corners[2] );
  vp.DrawLine( m_corners[2], m_corners[3] );
  vp.DrawLine( m_corners[3], m_corners[0] );
  vp.DrawPoint( m_line.from );
  vp.DrawPoint(  m_line.to );
  CRhinoGetXform::DynamicDraw( hdc, vp, point );
}
