...
CRhinoCommand::result CCommandSampleMakeBlock::RunCommand(const CRhinoCommandContext& context)
{
  ...
  if (idef_index >= 0)
  {
    RhinoApp().Print(L"Block definition \"%ls\" already exists.\n", static_cast<const wchar_t*>(idef_name));
    return CRhinoCommand::nothing;
  }
  ...
  ON_Xform xform = ON_Xform::TranslationTransformation(ON_origin - base_point);
  ...
  if (idef_index < 0)
  {
    RhinoApp().Print(L"Unable to create block definition \"%ls\".\n", static_cast<const wchar_t*>(idef_name));
    return CRhinoCommand::failure;
  }
