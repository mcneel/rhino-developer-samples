...
{
  const ON_MeshParameters& mp = context.m_doc.Properties().MeshParameters( ON_MeshParameters::render_mesh_custom );
  ...
  ON_MeshParameters new_mp( mp );
  new_mp.Set( new_relative_tolerance );

  context.m_doc.Properties().SetCustomRenderMeshParameters( new_mp );
