...
void CRhinoUiDialogScrollHelper::SetDisplaySize( int width, int height )
{
...
void CRhinoUiDialogScrollHelper::EnableScrollBars( bool bEnable )
{
  EnableScrollBar( SB_HORZ, bEnable );
  EnableScrollBar( SB_VERT, bEnable );
}

void CRhinoUiDialogScrollHelper::EnableScrollBar( int nBar, bool bEnable )
{
  if( m_pWnd && ::IsWindow(m_pWnd->m_hWnd) )
    m_pWnd->EnableScrollBar( nBar, bEnable ? ESB_ENABLE_BOTH : ESB_DISABLE_BOTH );
}

void CRhinoUiDialogScrollHelper::ScrollToOrigin( bool bScrollLeft, bool bScrollTop )
{
  if( 0 == m_pWnd )
    return;
...
void CRhinoUiDialogScrollHelper::OnHScroll( UINT nSBCode, UINT nPos, CScrollBar* pScrollBar )
{
  if( 0 == m_pWnd )
    return;
...
void CRhinoUiDialogScrollHelper::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
  if( 0 == m_pWnd )
    return;
...
BOOL CRhinoUiDialogScrollHelper::OnMouseWheel( UINT nFlags, short zDelta, CPoint pt )
{
  if( 0 == m_pWnd )
    return FALSE;
...
void CRhinoUiDialogScrollHelper::OnSize( UINT nType, int cx, int cy )
{
  UpdateScrollInfo();
}

int CRhinoUiDialogScrollHelper::Get32BitScrollPos( int nBar, CScrollBar* pScrollBar )
{
  if( 0 == m_pWnd )
    return 0;
...
void CRhinoUiDialogScrollHelper::UpdateScrollInfo()
{
  if( 0 == m_pWnd )
    return;
...
void CRhinoUiDialogScrollHelper::UpdateScrollBar( int nBar, int window_size, int display_size, LONG& page_size, LONG& scroll_pos, LONG& delta_pos )
{
  int scroll_max = 0;
  delta_pos = 0;
