...
RHINO_PLUG_IN_NAME( L"SampleRdkContentIOPlugIn" );

// Rhino plug-in id
// Provide a unique uuid for this plug-in
RHINO_PLUG_IN_ID( L"81B03B43-D57D-4D71-A2D7-00612F4C23AD" );
...
RHINO_PLUG_IN_DESCRIPTION(L"SampleRdkContentIOPlugIn plug-in for Rhinoceros®");
...
// The one and only CSampleRdkContentIOPlugInRhinoPlugIn object
static CSampleRdkContentIOPlugInRhinoPlugIn thePlugIn;

CSampleRdkContentIOPlugInRhinoPlugIn& SampleRdkContentIOPlugInRhinoPlugIn()
{
  // Return a reference to the one and only CSampleRdkContentIOPlugInRhinoPlugIn object
  return thePlugIn;
}

CSampleRdkContentIOPlugInRhinoPlugIn::CSampleRdkContentIOPlugInRhinoPlugIn()
{
	// Description:
	//   CSampleRdkContentIOPlugInRhinoPlugIn constructor. The constructor is called when the
	//   plug-in is loaded and "thePlugIn" is constructed. Once the plug-in
	//   is loaded, CSampleRdkContentIOPlugInRhinoPlugIn::OnLoadPlugIn() is called. The
...
{
	// Description:
	//   CSampleRdkContentIOPlugInRhinoPlugIn destructor. The destructor is called to destroy
	//   "thePlugIn" when the plug-in is unloaded. Immediately before the
	//   DLL is unloaded, CSampleRdkContentIOPlugInRhinoPlugIn::OnUnloadPlugin() is called. Do
...
const wchar_t* CSampleRdkContentIOPlugInRhinoPlugIn::PlugInName() const
{
	// Description:
...
const wchar_t* CSampleRdkContentIOPlugInRhinoPlugIn::PlugInVersion() const
{
	// Description:
...
GUID CSampleRdkContentIOPlugInRhinoPlugIn::PlugInID() const
{
	// Description:
...
BOOL CSampleRdkContentIOPlugInRhinoPlugIn::OnLoadPlugIn()
{
	// Description:
	...
	m_pRdkPlugIn = new CSampleRdkContentIOPlugInRdkPlugIn;
...
void CSampleRdkContentIOPlugInRhinoPlugIn::OnUnloadPlugIn()
{
	// Description:
...
CRhinoCommand::result CSampleRdkContentIOPlugInRhinoPlugIn::Render(const CRhinoCommandContext&, bool)
{
...
BOOL CSampleRdkContentIOPlugInRhinoPlugIn::AddToPlugInHelpMenu() const
{
	// Description:
...
BOOL CSampleRdkContentIOPlugInRhinoPlugIn::OnDisplayPlugInHelp( HWND hWnd ) const
{
	// Description:
