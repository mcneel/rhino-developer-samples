...
/*	This example of a 'Content I/O plug-in' demonstrates how to load a Basic Material
	(AKA Custom Material) from a simple file format. The example uses a file
	format with an extension of '.sample' and a simple name=value style grammar.
	Note that some types supported by this loader such as vector-2d do not actually
	occur in Basic Materials but are included for completeness.
...
CRhRdkContentKindList CSampleRdkContentIOPlugIn::SupportedKinds(void) const
{
...
UUID CSampleRdkContentIOPlugIn::PlugInId(void) const
{
	return CSampleRdkContentIOPlugInRdkPlugIn::RhinoPlugInId();
}

UUID CSampleRdkContentIOPlugIn::RdkPlugInId(void) const
{
...
bool CSampleRdkContentIOPlugIn::Parse(char*& p, const char* pSubString, bool bExpect) const
{
	SkipWhiteSpace(p);
	...
	if ((nullptr == pFound) || (pFound != p))
	{
		...
		{
			ON_wString s;
			s.Format(L"CSampleRdkContentIOPlugIn syntax error while loading file\n\n%ls\n\n"
...
bool CSampleRdkContentIOPlugIn::ParseBool(char*& p, bool& value) const
{
	SkipWhiteSpace(p);
...
bool CSampleRdkContentIOPlugIn::ParseDouble(char*& p, double& value) const
{
	SkipWhiteSpace(p);
...
bool CSampleRdkContentIOPlugIn::ParseFloat(char*& p, float& value) const
{
	double d = 0.0;
...
bool CSampleRdkContentIOPlugIn::ParseFloatList(char*& p, float* a, int num) const
{
	for (int i  = 0; i < num; i++)
	{
		if (!ParseFloat(p, a[i]))
...
CRhRdkVariant CSampleRdkContentIOPlugIn::GetValue(char* p) const
{
...
CRhRdkContent* CSampleRdkContentIOPlugIn::Load(const CRhinoDoc* pDoc, const wchar_t* wszFilename, CRhRdkContent::Kinds /*kind*/) const
{
	// Note: You must NOT attach your loaded content to a document.
	...
	while (0 != *p)
	{
		...
		if (('\r' == c) || ('\n' == c))
		{
			...
			*p = c;
			...
			if (!sName.IsEmpty())
			{
				// Name is not empty so set the corresponding parameter
				// of the basic material. This relies on the strings matching
				// the field names of the basic material as specified in
				// the RDK SDK header RhRdkFieldStrings.h
				pMaterial->SetParameter(sName, vValue);
			}
...
bool CSampleRdkContentIOPlugIn::Save(const wchar_t* wszFilename, const CRhRdkContent& c, const IRhRdkPreviewSceneServer*) const
{
	const ON_String sFilename = wszFilename;
