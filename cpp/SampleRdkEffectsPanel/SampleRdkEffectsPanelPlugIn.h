...
class CSampleRdkEffectsPanelRdkPlugIn;

class CSampleRdkEffectsPanelPlugIn : public CRhinoRenderPlugIn
{
public:
	...
	CSampleRdkEffectsPanelPlugIn();

	// CSampleRdkEffectsPanelPlugIn destructor. The destructor is called to destroy
	// "thePlugIn" when the plug-in is unloaded. Immediately before the
	// DLL is unloaded, CSampleRdkEffectsPanelPlugIn::OnUnloadPlugin() is called. Do
	// not do too much here. Be sure to clean up any memory you have allocated
	// with onmalloc(), onrealloc(), oncalloc(), or onstrdup().
	virtual ~CSampleRdkEffectsPanelPlugIn() = default;
	...
	// Render methods
//	CRhinoCommand::result RenderQuiet(const CRhinoCommandContext& context, bool bPreview) override;
//	BOOL SceneChanged() const override;
//	void SetSceneChanged(BOOL bChanged) override;
//	BOOL LightingChanged() const override;
//	void SetLightingChanged(BOOL bChanged) override;
//	UINT MainFrameResourceID() const override;

private:
	ON_wString m_plugin_version;
	class CSampleRdkEffectsPanelRdkPlugIn* m_pRdkPlugIn;
